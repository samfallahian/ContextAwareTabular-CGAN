...
from ctgan.models.gan_model import Generator, Discriminator
import torch
...
class CTGAN(BaseModel):
    ...
    def __init__(self, embedding_dim=156, generator_dim=(256, 256), discriminator_dim=(256, 256),
        ...
        for i in range(epochs):
            ...
            if (i + 1) % self.save_interval == 0:
                torch.save(self._generator.state_dict(), os.path.join(self.save_directory,
                                                                      f"generator_model_checkpoint_{i + 1}_{datetime.now().date().strftime('%m%d%Y')}_{self._device}.pth"))

        if self.save_directory:
            # Save the final trained model
            torch.save(self._generator.state_dict(), os.path.join(self.save_directory,
                                                                  f"generator_final_saved_model_{datetime.now().date().strftime('%m%d%Y')}_{self._device}.pth"))
            # Save training result
            self.df_result.to_csv(os.path.join(self.save_directory,
                                               f"gan_logs_{datetime.now().date().strftime('%m%d%Y')}.csv"),
                                  index=False)
